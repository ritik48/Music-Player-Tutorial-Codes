...
class MediaPlayer:
    def __init__(self, window):
        ...
        self.play_button = tk.Button(self.root,image=self.play_icon,command="command",cursor='hand2',bd=0,
        ...
        m1.add_command(label="Add Song",command="command",image=self.add_song_icon,compound="left")
        m1.add_command(label="Add Multiple Songs", command="command",image=self.multiple_song_icon,compound="left")

        m2 = tk.Menu(self.menu, background="grey", tearoff=False, bd=0, activebackground="black")
        self.menu.add_cascade(label="Delete", menu=m2)

        m2.add_command(label="Delete", command="command", image=self.delete_icon, compound="left")
